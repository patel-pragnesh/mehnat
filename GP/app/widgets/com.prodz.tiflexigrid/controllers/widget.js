...
var init = function(opts) {
	...
	screenWidth = params.width || Ti.Platform.displayCaps.getPlatformWidth() * 0.61;
	if (OS_ANDROID) {
	...
	onItemLongPress = params.onItemLongPress ||
	function() {
		Ti.API.info('TiFlexiGrid -> onItemClick is not defined.');
	};
...
var homeGridHeight = Ti.Platform.displayCaps.getPlatformHeight() * 0.12;
var popupHeight = Ti.Platform.displayCaps.getPlatformHeight() * 0.8;
var modifierHeight = popupHeight * 0.14;
var couponHeight = popupHeight * 0.08;

//Loyalty Coupon Dimensions
var loyaltyHeight = Ti.Platform.displayCaps.getPlatformHeight() * 0.25;
var addGridItem = function(item) {

	var frame = Ti.UI.createView({
		width : (from == "custom") ? "18.5%" : (from == "discount") ? "22%" : (from == "loyalty") ? loyaltyHeight : "31.65%",
		height : (from == "custom") ? modifierHeight : (from == "discount") ? couponHeight : (from == "loyalty") ? loyaltyHeight : homeGridHeight,
		backgroundColor : (from == "custom") ? "#EEEEEE" : "white",
		top : 0,
		left : 0,
		right : space,
		bottom : space,
	...
	var overlay = Ti.UI.createButton({
		width : Ti.UI.FILL,
		height : Ti.UI.FILL,
		...
		data : item.data,
		vw : item.view,
		touchEnabled : (from == "custom") ? item.enabled : true
	});

	var gridElement = item.view;

	//ADD CUSTOM FUNCTION ONCE AN ITEM IS CLICKED
	overlay.addEventListener('singletap', function(e) {
		onItemClick(e);
	});
	overlay.addEventListener('longpress', function(e) {
		onItemLongPress(e);
	});
...
exports.setOnItemLongPress = setOnItemLongPress;
