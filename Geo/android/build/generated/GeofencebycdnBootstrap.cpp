...
#define TAG "com.cdn.geofence"

using namespace v8;

static Persistent<Object> bindingCache;

static void Geofencebycdn_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Geofencebycdn.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::GeofencebycdnBindings::lookupGeneratedInit(
...
static void Geofencebycdn_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Geofencebycdn_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Geofencebycdn_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::GeofencebycdnBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry GeofencebycdnBinding = {
	"com.cdn.geofence",
	Geofencebycdn_init,
	Geofencebycdn_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_cdn_geofence_GeofencebycdnBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.cdn.geofence", &GeofencebycdnBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::GeofencebycdnBindings::lookupGeneratedInit));
}
