...
@property (strong, nonatomic) BSBonjourServer *bonjourServer;
...
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"title" message:[NSString stringWithFormat:@"test array %d", _foundServices.count] delegate:self cancelButtonTitle:@"cancel" otherButtonTitles:nil];
//    [alert show];
    NSLog(@"test array %d",_foundServices.count);
    NSLog(@"Connecting to service at index = %ld, count = %lu", (long)index, (unsigned long)_foundServices.count);
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)aNetServiceBrowser didFindService:(NSNetService *)aNetService moreComing:(BOOL)moreComing {
    ...
    if ([self.foundServices indexOfObject:aNetService] == NSNotFound) {
        if(![self.foundServices containsObject:aNetService]){

//            NSMutableDictionary *periferal_data = [[NSMutableDictionary alloc] init];
//            [periferal_data setObject:aNetService.name forKey:@"name"];
//            [periferal_data setObject:@"Service" forKey:@"type"];
            [self.foundServices addObject:aNetService];
//             [self.foundServices addObject:periferal_data];
            NSLog(@"testting");
        }
    }
